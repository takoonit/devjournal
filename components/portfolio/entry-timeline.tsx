            ...
            <div className="flex flex-wrap gap-2">
                {FILTERS.map((item) => (
                    <button
                        ...
                        className={`rounded-full border px-3 py-1 text-xs uppercase tracking-wide transition-colors ${filter === item.value ? "border-brand-400/40 bg-brand-400/15 text-brand-400" : "border-zinc-700 text-zinc-400 hover:border-zinc-500 hover:text-zinc-200"}`}
                    >
                        {item.label}
                    </button>
                ))}
