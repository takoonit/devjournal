...
- [ ] 2.5 Make edits to confirmed public entries update Supabase before local state and retain the edit draft when the remote update fails.
- [ ] 2.6 Make public project and profile edits update Supabase before closing or reporting success; keep private-only edits local.
- [ ] 2.7 Make deletion of public entries and projects remote-first while keeping private deletion local and preserving the existing confirmation dialogs.
- [ ] 2.8 Replace project-publishing claims with entry-level visibility copy and show `View public page` only when the project has a confirmed public entry.
